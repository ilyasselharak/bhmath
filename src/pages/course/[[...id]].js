    ...
    <div >
       ...
       <div className='flex flex-col items-center gap-y-12'>
        {filtred[0].courseLink.split(",,").map((item,index)=>{
          return <iframe key={index} src={item.trim()} style={{width:"80%"}} height="780" allow="autoplay" ></iframe>}
          )}
         </div>
        :
<div className=' flex justify-between mt-16'>
  <Image src={'/ads600.jpg'} width={160} height={600} alt={"addsenceAds"}/>
        <div className='flex flex-col gap-y-5  uppercase '>{course.map((item,index)=><Link key={index} href={`${router.asPath}/${item.name}`}><div  className=' bg-yellow-200 p-4 rounded-md text-center'>{item.name}</div></Link>)}</div>
        <Image src={'/ads600.jpg'} width={160} height={600} alt={"addsenceAds"}/>
        </div>}
    </div>
    </>
